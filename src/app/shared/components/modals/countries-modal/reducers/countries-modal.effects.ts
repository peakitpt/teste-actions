...
@Injectable()
export class CountriesModalEffects {
  @Effect()
  RequestGetAll = this.actions$.pipe(
    ofType(actions.CountriesActionTypes.RequestGetAll),
    map((action: actions.SuccessGetAll) => action.payload),
    switchMap((payload: any) => {
      return this.service.getAll(payload).pipe(
        map(
          (response: CountriesResponse) => new actions.SuccessGetAll(response)
        ),
        catchError(error =>
          of(
            new actions.RequestFail(
              new RequestError(
                actions.CountriesActionTypes.RequestGetAll,
                error
              )
            )
          )
  ...
  RequestSetSelected = this.actions$.pipe(
    ofType(actions.CountriesActionTypes.RequestSetSelected),
