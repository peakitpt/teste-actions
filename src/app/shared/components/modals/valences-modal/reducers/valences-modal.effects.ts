...
@Injectable()
export class ValencesModalEffects {
  @Effect()
  RequestGetAll = this.actions$.pipe(
    ofType(actions.ValencesActionTypes.RequestGetAll),
    map((action: actions.SuccessGetAll) => action.payload),
    switchMap((payload: any) => {
      return this.service.getAll(payload).pipe(
        map(
          (response: ValencesResponse) => new actions.SuccessGetAll(response)
        ),
        catchError((error) =>
          of(
            new actions.RequestFail(
              new RequestError(actions.ValencesActionTypes.RequestGetAll, error)
            )
          )
        )
      );
  ...
  RequestSetSelected = this.actions$.pipe(
    ofType(actions.ValencesActionTypes.RequestSetSelected),
