...
export class ChapelriesModalEffects {
  ...
  RequestGetAll = this.actions$.pipe(
    ...
    switchMap((payload: any) => {
      return this.service.getAll(payload).pipe(
        map(
          (response: ChapelriesResponse) => new actions.SuccessGetAll(response)
        ),
        catchError((error) =>
          of(
  ...
  RequestSetSelected = this.actions$.pipe(
    ofType(actions.ChapelriesActionTypes.RequestSetSelected),
  ...
  constructor(private actions$: Actions, private service: ChapelriesModalService) { }
}
