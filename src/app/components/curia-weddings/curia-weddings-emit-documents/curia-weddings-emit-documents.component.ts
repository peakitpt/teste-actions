...
@Component({
  selector: 'kyr-curia-weddings-emit-documents',
  templateUrl: './curia-weddings-emit-documents.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CuriaWeddingsEmitDocumentsComponent
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
    ...
    this.subs.push(
      this.route.params.subscribe((params) => {
        ...
        if (params.id && this.preFillWithNew) {
          ...
          this.subs.push(
            this.service
              .getNewCuriaWeddingDocument(this.id)
              .subscribe((obj: any) => {
                if (obj) {
  ...
  onFormValid(payload = this.form.getRawValue()) {
    this.subs.push(
      this.service.createNewCuriaWeddingDocument(this.id, payload).subscribe(
        (response: any) => {
