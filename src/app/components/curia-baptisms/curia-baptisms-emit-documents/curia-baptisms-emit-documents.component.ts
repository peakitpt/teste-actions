...
@Component({
  selector: 'kyr-curia-baptisms-emit-documents',
  templateUrl: './curia-baptisms-emit-documents.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CuriaBaptismsEmitDocumentsComponent
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
    ...
    this.subs.push(
      this.route.params.subscribe((params) => {
        ...
        if (params.id && this.preFillWithNew) {
          ...
          this.subs.push(
            this.service
              .getNewCuriaBaptismDocument(this.id)
              .subscribe((obj: any) => {
                if (obj) {
  ...
  onFormValid(payload = this.form.getRawValue()) {
    this.subs.push(
      this.service.createNewCuriaBaptismDocument(this.id, payload).subscribe(
        (response: any) => {
