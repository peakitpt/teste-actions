...
@Component({
  selector: 'kyr-curia-provisions-emit-documents',
  templateUrl: './curia-provisions-emit-documents.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CuriaProvisionsEmitDocumentsComponent
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
    ...
    this.subs.push(
      this.route.params.subscribe((params) => {
        ...
        if (params.id && this.preFillWithNew) {
          ...
          this.subs.push(
            this.service
              .getNewCuriaProvisionDocument(this.id)
              .subscribe((obj: any) => {
                if (obj) {
  ...
  onFormValid(payload = this.form.getRawValue()) {
    this.subs.push(
      this.service.createNewCuriaProvisionDocument(this.id, payload).subscribe(
        (response: any) => {
