...
@Component({
  selector: 'kyr-curia-secretariats-emit-documents',
  templateUrl: './curia-secretariats-emit-documents.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CuriaSecretariatsEmitDocumentsComponent
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
    ...
    this.subs.push(
      this.route.params.subscribe((params) => {
        ...
        if (params.id && this.preFillWithNew) {
          ...
          this.subs.push(
            this.service
              .getNewCuriaSecretariatDocument(this.id)
              .subscribe((obj: any) => {
                if (obj) {
  ...
  onFormValid(payload = this.form.getRawValue()) {
    this.subs.push(
      this.service
        .createNewCuriaSecretariatDocument(this.id, payload)
        .subscribe(
          (response: any) => {
