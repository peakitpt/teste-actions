...
@Component({
  selector: 'kyr-curia-economic-councils-emit-documents',
  templateUrl: './curia-economic-councils-emit-documents.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CuriaEconomicCouncilsEmitDocumentsComponent
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
    ...
    this.subs.push(
      this.route.params.subscribe((params) => {
        ...
        if (params.id && this.preFillWithNew) {
          ...
          this.subs.push(
            this.service
              .getNewCuriaEconomicCouncilDocument(this.id)
              .subscribe((obj: any) => {
                if (obj) {
  ...
  onFormValid(payload = this.form.getRawValue()) {
    this.subs.push(
      this.service
        .createNewCuriaEconomicCouncilDocument(this.id, payload)
        .subscribe(
          (response: any) => {
